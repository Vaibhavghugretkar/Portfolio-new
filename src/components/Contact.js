...
export const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    let response = await fetch("vaibhavghugretkar.netlify.app/contact", {
      method: "POST",
      headers: {
                ...
                <form onSubmit={handleSubmit}>
                  <Row>
                    ...
                    <Col size={12} className="px-1">
                      <textarea rows="6" style={{ resize: 'none' }} value={formDetails.message} placeholder="Message" onChange={(e) => onFormUpdate('message', e.target.value)} required></textarea>
                  ...
                  </Row>
                  {
                      status.message &&
                      <Col>
                        <p className={status.success === false ? "danger" : "success"} style={{ color: "white" , padding:"2px"}} >{status.message}</p>
                      </Col>
                    }
                </form>
              </div>}
